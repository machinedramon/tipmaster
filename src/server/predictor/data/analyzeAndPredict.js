...
// Função para carregar dados de patterns.json
const loadPatterns = () => {
  const patterns = JSON.parse(readFileSync("./patterns.json"));
  return patterns;
};
...
// Função para executar múltiplas previsões e contar os resultados
const runMultiplePredictions = (records, iterations) => {
  let redCount = 0;
  let blackCount = 0;
  let whiteCount = 0;

  for (let i = 0; i < iterations; i++) {
    const predictedColor = analyzeAndPredict(records);
    if (predictedColor === "red") {
      redCount++;
    } else if (predictedColor === "black") {
      blackCount++;
    } else if (predictedColor === "white") {
      whiteCount++;
    }
  }

  console.log(`Results after ${iterations} predictions:`);
  console.log(`Red: ${redCount}`);
  console.log(`Black: ${blackCount}`);
  console.log(`White: ${whiteCount}`);
};

// Executar a análise e previsão com os dados do arquivo especificado
const inputFilePath = "./data.json"; // Altere para o caminho do seu arquivo de entrada
const records = loadData(inputFilePath);
runMultiplePredictions(records, 1000);

export default analyzeAndPredict;
